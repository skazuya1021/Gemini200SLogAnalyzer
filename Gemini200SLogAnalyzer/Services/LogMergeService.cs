...
public sealed class LogMergeService
{
        ...
        CancellationToken cancellationToken = default)
    {
        var sortedPaths = filePaths
            .Select(p => new { Path = p, Date = LogFileParser.ExtractSortDate(p) })
            .OrderBy(x => x.Date)
        ...
        {
            throw new InvalidOperationException("処理対象のログファイルがありません。");
        }

        var total = sortedPaths.Count;
        var parsedFiles = new ParsedLogFile[total];
        var processed = 0;
        ...
        await Task.Run(() =>
        {
            Parallel.ForEach(
                ...
                item =>
                {
                    ...
                    parsedFiles[item.index] = LogFileParser.Parse(item.path);
                    var count = Interlocked.Increment(ref processed);
                    progress?.Report((count, total, $"読み込み中: {Path.GetFileName(item.path)}"));
                });
        }, cancellationToken);
        ...
        for (var fileIndex = 0; fileIndex < orderedParsed.Count; fileIndex++)
        {
            ...
            progress?.Report((fileIndex + 1, total, $"合体処理中: {parsed.FileName}"));
